...
class newEvaluationMethod:
	...
	def find_path (self, inputTrace):
		...
		while i < len(inputTrace):
			# if (i%500 == 0):
			# print("i = ", i, " length = ", len(inputTrace), end=" ")
			# elapsed_time = time.time() - start_time
			# msg = "Time took: %s secs" % timedelta(milliseconds=round(elapsed_time*1000))
			# print(msg)
			# start_time = time.time()
				...
				for terminal in self.startAndEnds[inputTrace[i]]:
					if terminal in inputTrace[i:len(inputTrace)]:
						...
						if remove != 0:
							...
							# test = []
							# for aMessage in remove:
							# 	print(aMessage+i, end=" ")
							# print()
							...
							elapsed_time = time.time() - start_time
							# msg = "Took: %s secs" % timedelta(milliseconds=round(elapsed_time*1000))
							# print(msg)
							print("Total removed till now = ", totalNumberOfAccepted, " and index = ", i, " time = ", timedelta(milliseconds=round(elapsed_time*1000)))
							break
						else:
							foundPathFlag = False
